...
RealType right_pvalue(RealType mean, UIntType n) {
    return boost::math::gamma_p(RealType(n), mean);
}
...
// Knuth (1981) suggested n=2^14 and m=2^20 and hence m/n = 64, but the
// p-value estimates looks then strongly biased towards 1. In practice though,
// using m/n=64 does not change the thresholds at which the different methods
// break down.
template<typename RealType, typename UIntType>
struct Experiment
{
...
int main() {
    ...
    Cdf<RealType> cdf;
    RqsNormalDistribution<RealType, W, 7> rqs_dist;
    ...
    experiment.run(min_dim, max_dim, repeat,
        [&]() { return rqs::generate_random_real<RealType, W>(rng); });

    std::cout << "Statistics for ziggurat." << std::endl;
    experiment.run(min_dim, max_dim, repeat,
        [&]() { return cdf(ziggurat_dist(rng)); });

    std::cout << "Statistics for RQS." << std::endl;
    experiment.run(min_dim, max_dim, repeat,
        [&]() { return cdf(rqs_dist(rng)); });
}
